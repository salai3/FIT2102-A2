...
int = Parser f
  where
    ...
    f "" = Error UnexpectedEof
    f x@(c : _) = case readInt x of
        Just (v, rest) -> Result rest v
        Nothing -> Error $ UnexpectedChar c
...
-- | Write a parser that will parse a non-newline whitespace character.
--
